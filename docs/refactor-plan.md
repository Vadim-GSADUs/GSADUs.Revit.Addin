...
- Default path is **mutate catalog -> save to ES via document-scoped ExternalEvent -> reload from ES -> bump version/clear dirty -> notify subscribers**.
- "LoadFromES before Save" is **not** the default and should only exist if explicit merge/conflict logic is added; in the current single-user model, pre-loads risk reintroducing stale state and nondeterminism.

**Lifecycle & scoping**
- Maintain a document-scoped container keyed by **`Document.UniqueId` (preferred for stability across sessions; `DocumentId` acceptable if disposed on close)** that supplies catalog, ES provider, save orchestrator, and notifier.
- Wire disposal to Revit's document lifecycle (e.g., subscribe to `Application.DocumentClosed` in the ExternalApplication `OnStartup`) to remove the keyed entry, unsubscribe notifiers, clear queues, and dispose the ExternalEvent when the document closes.
